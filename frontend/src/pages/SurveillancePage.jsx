...
import { getTasks, updateTaskAssignment, updateTaskStatus } from "../services/api.js";
...
function SurveillancePage({ onConnectionChange }) {
  ...
  async function assign(task, role) {
    const userIds = { ASHA_WORKER: "USR-ASHA-001", WATER_WORKER: "USR-WATER-001", GOVT_OFFICER: "USR-GOV-001" };
    setState((old) => ({ ...old, updating: task.id, notice: "", error: "" }));
    try {
      const updated = await updateTaskAssignment(task.id, role || null, role ? userIds[role] : null);
      setState((old) => ({ ...old, updating: "", tasks: old.tasks.map((item) => item.id === task.id ? updated : item), notice: `${task.id} assignment updated.` }));
    } catch (error) { setState((old) => ({ ...old, updating: "", error: error.message })); }
  }

  return (
    <div className="px-5 py-7 lg:px-8 lg:py-8"><p className="text-sm font-semibold text-violet-700">Protected demo session workflow</p><h1 className="mt-1 text-3xl font-bold tracking-tight">Surveillance & verification</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">Manage role-assigned evidence-verification tasks. Status and assignment changes reset when the backend restarts.</p>
      ...
      {state.status === "loading" ? <div className="mt-6 h-96 animate-pulse rounded-2xl bg-white" role="status"><span className="sr-only">Loading verification tasks</span></div> : <section className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="overflow-x-auto"><table className="w-full min-w-[1250px] text-left"><caption className="sr-only">Verification task workflow</caption><thead className="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500"><tr><th className="px-5 py-4">Task / village</th><th className="px-4 py-4">Type</th><th className="px-4 py-4">Reason</th><th className="px-4 py-4">Priority</th><th className="px-4 py-4">Created</th><th className="px-4 py-4">Assigned to</th><th className="px-4 py-4">Status</th><th className="px-4 py-4">Action</th></tr></thead><tbody className="divide-y divide-slate-100">{state.tasks.map((task) => <tr className="align-top" key={task.id}><td className="px-5 py-4"><p className="font-bold text-slate-900">{task.id}</p><a className="mt-1 block text-sm font-semibold text-teal-700" href={`#/villages/${task.village_id}`}>{task.village_name}</a><p className="text-xs text-slate-400">{task.district}</p></td><td className="px-4 py-4 text-sm font-semibold text-slate-700">{task.task_type.replaceAll("_", " ")}<span className="mt-1 block text-xs text-slate-400">{task.task_category}</span></td><td className="max-w-md px-4 py-4"><p className="text-sm font-semibold text-slate-800">{task.action}</p><ul className="mt-2 list-disc pl-4 text-xs leading-5 text-slate-500">{task.reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul></td><td className="px-4 py-4 text-sm font-black">#{task.priority}</td><td className="px-4 py-4 text-sm text-slate-600">{new Date(task.created_at).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}</td><td className="px-4 py-4"><select className="rounded-lg border border-slate-300 px-2 py-2 text-xs font-bold" disabled={state.updating === task.id} value={task.assigned_role || ""} onChange={(event) => assign(task, event.target.value)}><option value="">Unassigned</option><option value="GOVT_OFFICER">Government Officer</option>{task.task_category === "HEALTH" && <option value="ASHA_WORKER">ASHA Worker</option>}{task.task_category === "WATER" && <option value="WATER_WORKER">Water Worker</option>}</select></td><td className="px-4 py-4"><span className={`rounded-md px-2 py-1 text-[11px] font-bold ${statusStyles[task.status]}`}>{task.status.replaceAll("_", " ")}</span></td><td className="px-4 py-4">{nextStatus[task.status] ? <button className="rounded-lg bg-slate-950 px-3 py-2 text-xs font-bold text-white disabled:opacity-50" disabled={state.updating === task.id} onClick={() => advance(task)}>{state.updating === task.id ? "Updating…" : actionLabels[task.status]}</button> : <span className="text-xs font-semibold text-slate-400">Workflow complete</span>}</td></tr>)}</tbody></table></div></section>}
